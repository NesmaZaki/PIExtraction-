...
/**
 * This class takes a biconnected graph and decomposes it into the tree of the
 * triconnected components.<br/><br/>
 *
 * NOTE THAT GIVEN GRAPH MUST BE BICONNECTED; OTHERWISE RESULT IS
 * UNEXPECTED.<br/><br/>
 *
 * Every triconnected component is either of a trivial, or POLYGON, or BOND, or
 * RIGID type ({@link TCType}). Note that every edge is a trivial component, but
 * is not explicitly computed by this class. This implementation is an adaption
 * of the algorithm implemented by Martin Mader. The general process of this
 * decomposition is described in his master's thesis.<br/><br/>
 *
 * For more information on the algorithm please refer to: Carsten Gutwenger and
 * Petra Mutzel: A Linear Time Implementation of SPQR-Trees. Graph Drawing 2000:
 * 77-90.
 *
 * @see {@link BiconnectivityCheck} for testing graph biconnectivity.
 * @see {@link TCType} provides enumeration for types of triconnected
 * components.
 *
...
public class TCTree<E extends IEdge<V>, V extends IVertex> extends AbstractTree<TCTreeNode<E, V>> {
     ...
     *
     * @param graph A graph to decompose.
     * @param backEdge An edge of the graph to use as a back edge. A
     * triconnected component that contains the edge will become the root of the
     * tree.
     */
     ...
     * @return <tt>true</tt> if the triconnected component contains the back
     * edge; otherwise <tt>false</tt>.
     */
    ...
    private void classifyComponents() {
        ...
        //System.out.println(" Classify Components ");
        for (TCTreeNode<E, V> n : this.getVertices()) {            
            boolean isloop = false;
            if (n.skeleton.countVertices() == 2) {
                Iterator<V> vs2 = n.skeleton.getVertices().iterator();
                V v2 = n.skeleton.getVertices().iterator().next();
               
                while (vs2.hasNext()) {
                    V v = vs2.next();    
                    if (v.getSize() == 2 && v.getT1().equals("XOR") == true) {
                        isloop = true;
                        break;
                    }
                }
                if (isloop) {
                    n.type = TCType.LOOP;
                    continue;
                } else if(!isloop && v2.getT1().equals("XOR") ==  true) {
                    n.type = TCType.XOR;
                    continue;
                }else
                {
    ...
    /**
     * Runs the different DFS algorithms and creates the triconnected components
     * based on the given graph and maps.
     */
    ...
    /**
     * This method is used for the deletion of multiple edges. The edges are
     * sorted in a manner, so that multiple edges are positioned consecutively
     * in the returned EdgeList.
     */
    @SuppressWarnings("unchecked")
     ...
     * @return Collection of the triconnected components of the given
     * {@link TCType} class.
     */
