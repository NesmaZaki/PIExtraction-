...
/**
 * This class takes a multi-terminal graph and computes its Refined Process
 * Structure Tree (RPST).<br/><br/>
 *
 * NOTE THAT GIVEN GRAPH MUST BE MULTI-TERMINAL; OTHERWISE RESULT IS
 * UNEXPECTED.<br/><br/>
 *
 * The RPST of a multi-terminal graph is a containment hierarchy of all
 * canonical fragments the graph. A fragment is a single-entry-single-exit
 * (SESE) subgraph of the graph. A fragment of the graph is canonical if it does
 * not overlap (on edges) with any other fragment of the graph. Every canonical
 * fragment is induced by a triconnected component of the graph,
 *
 ...
 * Artem Polyvyanyy, Jussi Vanhatalo, and Hagen Voelzer. Simplified Computation
 * and Generalization of the Refined Process Structure Tree. Proceedings of the
 * 7th International Workshop on Web Services and Formal Methods (WS-FM).
 * Hoboken, NJ, US, September 2010.
 *
 * @see {@link DirectedGraphAlgorithm.isMultiTerminal} for checking if a graph
 * is multi-terminal.
 *
 ...
 *
 * @assumption Given graph is multi-terminal, see
 * {@code DirectedGraphAlgorithms.isMultiTerminal}.
 */
...
int x = 0;
    ...
    protected Map<V, Integer> Size = null;
    ...
    public RPST(IDirectedGraph<E, V> graph) {
        ...
        this.Size = new HashMap<V, Integer>();
        this.diGraph = graph;
        this.normalizeGraph();
    ...
    private void normalizeGraph() {
        ...
        for (V v : this.diGraph.getVertices()) {
            ...
            if(v.getDescription().equals("SPLIT") == true){
                x = this.diGraph.getIncomingEdges(v).size();
                v.setSize(x);
            }
            
           this.ov2nv.put(v, this.normalizedGraph.addVertex(new Vertex(v.getName(),v.getT1(),v.getDescription(),v.getSize())));
           x = 0 ;
        }
    ...
    private void constructRPST() {
        ...
        if (this.tctree.getEdges().isEmpty()) {
            ...
            for (IDirectedEdge<TCTreeNode<DirectedEdge, Vertex>> edge : this.tctree.getEdges()) {
                ...
                if (this.tctree.isRoot(src)) {
                    this.root = rsrc;
                }
                if (this.tctree.isRoot(tgt)) {
                    this.root = rtgt;
                }
